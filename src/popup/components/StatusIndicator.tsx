...
export function StatusIndicator({ toasts, isLoading, connectedCount = 0, enabledCount = 0 }: StatusIndicatorProps) {
  ...
  const getStatusIcon = () => {
    if (isLoading) {
      return (
        <div className="animate-spin w-4 h-4 border-2 border-ai-info border-t-transparent rounded-full"></div>
      );
    }

    if (!latestToast) {
      return (
        <div className="w-4 h-4 rounded-full bg-ai-text-disabled"></div>
      );
    }
    ...
    switch (latestToast.type) {
      case TOAST_TYPES.SUCCESS:
        return (
          <div className="w-4 h-4 rounded-full bg-ai-success flex items-center justify-center">
            <svg className="w-2.5 h-2.5 text-ai-text-inverted" fill="currentColor" viewBox="0 0 20 20">
      ...
      case TOAST_TYPES.ERROR:
        return (
          <div className="w-4 h-4 rounded-full bg-ai-error flex items-center justify-center">
            <svg className="w-2.5 h-2.5 text-ai-text-inverted" fill="currentColor" viewBox="0 0 20 20">
      ...
      default:
        return (
          <div className="w-4 h-4 rounded-full bg-ai-info flex items-center justify-center">
            <svg className="w-2.5 h-2.5 text-ai-text-inverted" fill="currentColor" viewBox="0 0 20 20">
