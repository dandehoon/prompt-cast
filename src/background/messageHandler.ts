...
export class MessageHandler {
  ...
  async getSiteStatus(site: SiteConfig): Promise<SiteStatusType> {
    try {
      ...
      try {
        const isReady = await BackgroundMessaging.isContentScriptReady(tab.id);
        return isReady ? 'connected' : 'loading';
      } catch {
  ...
  private async sendToSingleSite(
    ...
    try {
      ...
      if (tabs.length > 0) {
        ...
        const response = await this.tabManager.sendMessageWithRetry(
          tab.id,
          'INJECT_MESSAGE',
          { message },
        );
