<script lang="ts">
  ...
  const statusMessage = $derived(latestToast ? latestToast.message : getDefaultMessage());
</script>
...
{#if !shouldHide}
  <div class="flex items-center space-x-2">
    ...
    {/if}
    <span class="text-xs opacity-70 max-w-48 truncate" style="color: var(--pc-text-secondary);">
      {statusMessage}
    </span>
  </div>
{/if}
